...
# Colourblind-friendly Wong/IBM palette for docking engines in dendrogram labels.
DOCKING_ENGINE_NAMES = ("gnina", "plants", "smina", "vina")
DEFAULT_ENGINE_COLORS: Dict[str, str] = {
    "gnina": "#785EF0",
    "plants": "#009E73",
    "smina": "#E69F00",
    "vina": "#0072B2",
...
def _require_matplotlib():
    '''Lazy matplotlib import for optional dendrogram plots.'''
...
def _engine_from_mol2_stem(path: str) -> Optional[str]:
    '''Infer docking engine from ``{engine}_{stem}.mol2`` filenames.'''
    stem = path.rsplit("/", maxsplit=1)[-1]
    if stem.lower().endswith(".mol2"):
        stem = stem[:-5]
    if "_" not in stem:
        return None
    prefix = _normalize_engine_name(stem.split("_", 1)[0])
    if prefix in DEFAULT_ENGINE_COLORS:
        return prefix
    return None


def _resolve_pose_engine(path_at_idx: str, pose_engine_map: Dict[str, str]) -> Optional[str]:
    '''Resolve the docking engine for a pose path used in the RMSD matrix.'''
    if path_at_idx in pose_engine_map:
        return _normalize_engine_name(pose_engine_map[path_at_idx])

    for pose_path, engine in pose_engine_map.items():
        if path_at_idx in pose_path or pose_path in path_at_idx:
            return _normalize_engine_name(engine)

    return _engine_from_mol2_stem(path_at_idx)


def build_pose_engine_map(
        pose_paths: List[str],
        source_engine_map: Optional[Dict[str, str]] = None,
    ...
    ----------
    pose_paths : List[str]
        Pose paths used as the RMSD matrix index/columns (typically MOL2 files).
    source_engine_map : Dict[str, str], optional
        Mapping from original docked-pose paths to engine names.
    path_aliases : Dict[str, str], optional
        Mapping from ``pose_paths`` entries back to their original pose paths.

    Returns
    -------
    Dict[str, str]
        Engine map keyed by ``pose_paths`` entries.
    '''
    source_engine_map = source_engine_map or {}
    path_aliases = path_aliases or {}
    resolved: Dict[str, str] = {}

    lookup: Dict[str, str] = {}
    for pose_path, engine in source_engine_map.items():
        lookup[str(pose_path)] = _normalize_engine_name(engine)

    for pose_path in pose_paths:
        candidates = [pose_path]
...
def cluster_rmsd(data: Union[Dict[str, Dict[str, float]], pd.DataFrame], algorithm: str = 'agglomerativeClustering', max_distance_threshold: float = 20.0, min_distance_threshold: float = 10.0, threshold_step: float = 0.1, outputPlot: str = "", molecule_name: str = "", pose_engine_map: Optional[Dict[str, str]] = None, engine_colors: Optional[Dict[str, str]] = None) -> Union[np.ndarray, int]:
    ...
    pose_engine_map : Dict[str, str], optional
        Mapping from pose file paths to engine names (``gnina``, ``plants``, ``smina``, ``vina``).
        Used for coloring labels in the plot. Only engines present in the data appear in the legend.
    engine_colors : Dict[str, str], optional
        Mapping from engine names to colors. Defaults to :data:`DEFAULT_ENGINE_COLORS`.
        ...
        if outputPlot != "":
            try:
                ...
                if engine_colors is None:
                    engine_colors = dict(DEFAULT_ENGINE_COLORS)
                else:
                    engine_colors = {
                        _normalize_engine_name(name): color
                ...
                if pose_engine_map and isinstance(data, pd.DataFrame):
                    ...
                    for i, label in enumerate(tick_labels):
                        if i < len(leaf_order):
                            ...
                            if original_idx < len(data.index):
                                path_at_idx = str(data.index[original_idx])
                                engine = _resolve_pose_engine(path_at_idx, pose_engine_map)
                ...
                # Add engine colors to legend for engines actually used in this plot
                if engines_used:
                    for engine_name in DOCKING_ENGINE_NAMES:
                        if engine_name in engines_used and engine_name in engine_colors:
