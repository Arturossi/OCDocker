    ...
    for dir in tqdm(iterable=dirs, total=len(dirs)):
        ...
        if not os.path.isdir(p2rankDir):
            ...
            if os.path.isdir(p2rankDir):
                ...
                octools.print_error_log(f"Unable to generate the p2rank dir for '{dir}'... Error code {errorCode}.", f"{logdir}/DUDEZ_integrity_report.log")
                continue
        ...
        if not os.path.isdir(vinaDir):
            ...
            if os.path.isdir(vinaDir):
                ...
                octools.print_error_log(f"Unable to generate the vinaFiles dir for '{dir}'... Error code {errorCode}.", f"{logdir}/DUDEZ_integrity_report.log")
                continue
        ...
        if boxCount == 0:
            ...
            if boxCount > 0:
                ...
                octools.print_error_log(f"The protein '{dir}' still has no box file.", f"{logdir}/DUDEZ_integrity_report.log")
                continue
        ...
        if len(glob(f"{dir}/vinaFiles/*")) == boxCount:
            ...
            if len(glob(f"{dir}/vinaFiles/*")) == boxCount:
                ...
                octools.print_error_log(f"Unable to generate the conf files dir for '{dir}'...", f"{logdir}/DUDEZ_integrity_report.log")
                continue

    octools.printv("Integrity check of the DUDEZ database accomplished.")
