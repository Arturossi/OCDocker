...
class ONet(nn.Module): # Sizes don't match (The number of encoder and decoder layers must match for upper and lower bounds)
    def __init__(self, in_channels, out_channels):
        super(ONet, self).__init__()
        ...
        self.encoder3_up = self.contracting_block(128, 256, apply_pooling=True)
        self.encoder4_up = self.contracting_block(256, 512, apply_pooling=True)
        ...
        self.decoder4_up = self.expansive_block(1024, 512, 256)
        self.decoder3_up = self.expansive_block(512, 256, 128)
    ...
    def forward(self, x):
        ...
        encoder3_up = self.encoder3_up(torch.cat([encoder2_up, encoder2_down], dim=1))

        encoder4_up = self.encoder4_up(encoder3_up)
        
        # Bottleneck
        bottleneck_up = self.bottleneck_up(encoder4_up)
        ...
        decoder2_up = self.decoder2_up(torch.cat([decoder3_up, encoder2_up, encoder2_down, decoder2_down], dim=1))

        decoder1_down = self.decoder1_down(decoder2_down)
        decoder1_up = self.decoder1_up(torch.cat([decoder2_up, encoder1_up, encoder1_down, decoder1_down], dim=1))
