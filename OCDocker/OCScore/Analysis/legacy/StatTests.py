...
import OCDocker.OCScore.Analysis.legacy.StatTests as ocstat
'''
...
LOGGER = oclogging.get_logger("ocscore.analysis.stat_tests")
...
def run_pca_analysis(
    ...
    for pca_type in ['80', '85', '90', '95']:
        ...
        if not os.path.exists(model_path):
            LOGGER.warning("PCA model not found: %s", model_path)
            continue
        ...
        try:
            ...
            LOGGER.info("Saved PCA feature importance for %s.", pca_type)
        except Exception as e:
            LOGGER.exception("Failed to process PCA feature importance for %s.", pca_type)
...
def run_statistical_tests(df: pd.DataFrame, n_trials: int, colour_mapping: dict[str, tuple[float, float, float]], output_dir: str) -> None:
    ...
    LOGGER.info("Running Welch's ANOVA and Games-Howell post-hoc tests.")
    ...
    LOGGER.info("Running normality and variance tests.")
    ...
    LOGGER.info("Statistical analysis complete. Results saved to 'csvs/' and 'plots/'.")
    plt.close('all')
