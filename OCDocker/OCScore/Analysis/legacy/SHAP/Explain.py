...
def compute_shap_values(
    ...
    '''Compute SHAP values for a neural network model using Deep, Gradient, Kernel, or Permutation explainer.
    ...
    explainer : str
        Type of SHAP explainer to use: "deep", "gradient", "kernel",
        or "permutation". Default is "gradient".
    ...
    def _model_predict_batch(x_numpy: np.ndarray) -> np.ndarray:
        '''Return one scalar prediction per row for KernelExplainer.'''

        batch = np.asarray(x_numpy, dtype=np.float32)
        if batch.ndim == 1:
            batch = batch.reshape(1, -1)
        x_tensor = torch.tensor(batch, dtype=torch.float32, device=device)
        with torch.no_grad():
            out = neural.NN(x_tensor).detach().cpu().numpy()
        return np.asarray(out, dtype=np.float64).reshape(-1)

    explainer_name = explainer.lower()
    if explainer_name == "deep":
        ...
        eval_tensor = torch.tensor(eval_np, dtype=torch.float32, device=device)
        try:
            shap_values = deep_explainer.shap_values(eval_tensor, check_additivity=False)
        except AssertionError:
            # Unsupported activations (e.g. SiLU) often fail additivity even when disabled.
            kernel_explainer = shap.KernelExplainer(_model_predict_batch, bkg_np)
            shap_values = kernel_explainer.shap_values(eval_np)
    elif explainer_name == "gradient":
        if not hasattr(shap, "GradientExplainer"):
            ocerror.Error.value_error("Installed SHAP does not provide GradientExplainer.")
            raise ValueError("SHAP GradientExplainer is unavailable")
        background_tensor = torch.tensor(bkg_np, dtype=torch.float32, device=device)
        gradient_explainer = shap.GradientExplainer(neural.NN, background_tensor)
        eval_tensor = torch.tensor(eval_np, dtype=torch.float32, device=device)
        shap_values = gradient_explainer.shap_values(eval_tensor)
    elif explainer_name == "kernel":
        kernel_explainer = shap.KernelExplainer(_model_predict_batch, bkg_np)
        shap_values = kernel_explainer.shap_values(eval_np)
    elif explainer_name == "permutation":
        if not hasattr(shap, "PermutationExplainer"):
            ocerror.Error.value_error("Installed SHAP does not provide PermutationExplainer.")
            raise ValueError("SHAP PermutationExplainer is unavailable")
        masker = bkg_np
        if hasattr(shap, "maskers") and hasattr(shap.maskers, "Independent"):
            masker = shap.maskers.Independent(bkg_np)
        permutation_explainer = shap.PermutationExplainer(_model_predict_batch, masker)
        explanation = permutation_explainer(eval_np)
        ...
        ocerror.Error.value_error(
            f"Invalid explainer type: '{explainer}'. Must be 'deep', 'gradient', 'kernel', or 'permutation'."
        )
        raise ValueError("explainer must be 'deep', 'gradient', 'kernel', or 'permutation'")
