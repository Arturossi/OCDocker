...
# Description
###############################################################################
''' Legacy module with a helper to perform Neural Network parameter
optimization using the previous Optuna protocol.
...
import OCDocker.OCScore.Optimization.legacy.DNN as ocdnn
'''
...
LOGGER = oclogging.get_logger("ocscore.optimization.dnn_legacy")
...
def optimize_NN(
    ...
    if autoencoder:
        if verbose:
            LOGGER.info("Running Auto Encoder optimization.")

        # If total_trials is not divisible by num_processes, warn the user
        if total_trials_autoencoder % num_processes_autoencoder != 0:
            LOGGER.warning(
                "total_trials_autoencoder is not divisible by num_processes_autoencoder; "
                "trials per process will be rounded down."
            )
    ...
    if run_NN_optimization:
        ...
        if verbose:
            LOGGER.info("Running Neural Network optimization.")

        # If total_trials is not divisible by num_processes, warn the user
        if total_trials_NN % num_processes_NN != 0:
            LOGGER.warning(
                "total_trials_NN is not divisible by num_processes_NN; "
                "trials per process will be rounded down."
            )
