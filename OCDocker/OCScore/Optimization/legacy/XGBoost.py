...
LOGGER = oclogging.get_logger("ocscore.optimization.xgboost")
...
def optimize_XGB(
    ...
    if run_GA_optimization:
        ...
        if total_trials_GA % num_processes_GA != 0:
            LOGGER.warning(
                "total_trials_GA is not divisible by num_processes_GA; "
                "trials per process will be rounded down."
            )
    ...
    if run_XGB_optimization:
        if verbose:
            LOGGER.info("Running XGBoost final optimization.")

        # If total_trials is not divisible by num_processes, warn the user
        if total_trials_XGB % num_processes_XGB != 0:
            LOGGER.warning(
                "total_trials_XGB is not divisible by num_processes_XGB; "
                "trials per process will be rounded down."
            )
