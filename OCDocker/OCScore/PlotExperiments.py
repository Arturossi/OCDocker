...
used_methods = ['Raw Scoring Function'] * len(methods) + ['Simple consensus'] * len(simple_methods_names) + [
        ...
        'PCA90 + NN', 'PCA90 + NN', 'PCA85 + NN', 'PCA85 + NN', 'PCA85 + NN', 
        'PCA85 + NN', 'PCA85 + NN', 'PCA85 + NN', 'PCA80 + NN', 'PCA80 + NN',
        'PCA80 + NN', 'PCA80 + NN', 'PCA80 + NN', 'PCA80 + NN', 'SF Only + NN',
        'SF Only + NN', 'SF Only + NN', 'SF Only + NN'
    ]
...
for m in to_remove:
    df_error_concat = df_error_concat[~df_error_concat['Metodologia'].str.startswith(m)]
    df_auc_concat = df_auc_concat[~df_auc_concat['Metodologia'].str.startswith(m)]
