...
# Convert all entries in 'best_combined_metric' to strings to avoid TypeError
data['best_combined_metric'] = data['best_combined_metric'].astype(str)
...
# Create the one-hot encoded features for the first 16 characters of the best_combined_metric column
...
for index, row in data.iterrows():
    ...
    ff = row['best_combined_features'][:16]
...
data.loc[features['best_combined_metric'] == features['best_combined_metric'].min(), 'highlight'] = 'best'
...
def run_elbow(normalized_features, max_clusters: int = 15, plot: bool = True):
...
def run_kmeans(normalized_features, optimal_clusters: int = 2):
    ...
    # Add edgecolor to Spearman correlation
    spearman_text.set_path_effects([path_effects.Stroke(linewidth=3, foreground='black'), path_effects.Normal()])
    plt.savefig('Kmeans.png')

    plt.close()

    return spearman_corr, p_value_spearman

# DBSCAN
###############################################################################

def run_dbscan(normalized_features):
    # Perform DBSCAN clustering
    dbscan = DBSCAN(eps=0.01, min_samples=10)
    normalized_features['cluster'] = dbscan.fit_predict(normalized_features[['best_combined_auc', 'best_combined_value']])

    # Calculate the Spearman correlation
    spearman_corr, p_value_spearman = spearmanr(normalized_features['best_combined_auc'], normalized_features['best_combined_value'])

    # Plot the clusters
    plt.figure(figsize=(14, 8))

    # Plot the elements with scatter plot
    sns.scatterplot(x='best_combined_auc', y='best_combined_value', hue='cluster', data=normalized_features, palette='viridis', edgecolor='black', s=50)

    # Add a regression line for Spearman correlation
    sns.regplot(x='best_combined_auc', y='best_combined_value', data=normalized_features, scatter=False, color='cyan')
...
def run_hdbscan(normalized_features, min_samples: int = 10, min_cluster_size: int = 10):
...
k_means_spearman_corr, k_means_p_value_spearman = run_kmeans(normalized_features, optimal_clusters = 4)
