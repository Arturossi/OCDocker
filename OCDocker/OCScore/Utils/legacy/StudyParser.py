...
###############################################################################
'''
Legacy parser for previous Optuna study metadata and summaries.

This module still understands the former mixed RMSE/AUC summary convention so
historical study outputs remain inspectable. Current staged Optuna reports are
task-separated in OCDocker.OCScore.Optimization.StagedOptuna.

Usage:

import OCDocker.OCScore.Utils.legacy.StudyParser as ocstudy
'''
...
LOGGER = oclogging.get_logger("ocscore.utils.study_parser")
...
def analyze_studies(
    ...
    for sname in snames:
        ...
        try:
            ...
            LOGGER.warning("Could not load %s: %s", sname, e)
            continue
...
def analyze_studies_old(
    ...
    for sname in snames:
        ...
        try:
            ...
            LOGGER.warning("Error loading study %s: %s", sname, e)
            continue
        ...
        for i in range(0, n_trials):
            ...
            if verbose:
                LOGGER.info(
                    "%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s",
                    len(df),
                    best_rmse_df["number"].iloc[i],
                    best_rmse_df["value"].iloc[i],
                    best_rmse_df["user_attrs_AUC"].iloc[i],
                    best_auc_df["number"].iloc[i],
                    best_auc_df["value"].iloc[i],
                    best_auc_df["user_attrs_AUC"].iloc[i],
                    best_df["number"].iloc[i],
                    best_df["combined_metric"].iloc[i],
                    best_df["user_attrs_AUC"].iloc[i],
                )
