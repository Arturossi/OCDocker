...
import OCDocker.OCScore.Utils.legacy.Workers as ocscoreworkers
'''
...
LOGGER = oclogging.get_logger("ocscore.utils.workers")

from OCDocker.OCScore.Optimization.legacy.models.dimensionality.AutoencoderOptimizer import AutoencoderOptimizer
from OCDocker.OCScore.Optimization.legacy.models.dimensionality.GeneticAlgorithm import GeneticAlgorithm
from OCDocker.OCScore.Optimization.legacy.models.dnn.DNNOptimizer import DNNOptimizer
from OCDocker.OCScore.Optimization.legacy.models.transformer.TransOptimizer import TransOptimizer
    ...
    *,
    ...
    for attempt in range(retries + 1):
        try:
            ...
            if verbose:
                LOGGER.warning(
                    "Transient Optuna storage contention during '%s' "
                    "(attempt %d/%d). Retrying in %.2fs. Error: %s: %s",
                    context,
                    attempt + 1,
                    retries + 1,
                    delay,
...
def AEworker(
    ...
    if verbose:
        LOGGER.info("Process %d starting optimization", pid)
    ...
    if verbose:
        LOGGER.info("Process %d has completed the optimization", pid)
...
def NNAblationworker(
    ...
    if verbose:
        LOGGER.info("Process %d starting ablation", pid)
    ...
    if isinstance(mask, list) and isinstance(mask[0], np.ndarray):
        ...
        if verbose:
            LOGGER.info("Process %d has completed the ablation", pid)
...
def Transworker(
        ...
        if verbose:
            LOGGER.info("Process %d has completed optimization", pid)
