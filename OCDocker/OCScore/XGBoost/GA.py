...
class EvolutionaryFeatureSelector:
    ...
    def __init__(self, X_train: np.ndarray, y_train: np.ndarray, X_test: np.ndarray, y_test: np.ndarray, params: dict, use_gpu: bool = False, random_state: int = 42, score_columns_index: list = []) -> None:
        '''
    ...
    def fitness_function(self, features: list) -> float:
        ...
        _, roc_auc = run_xgboost(self.X_train, self.y_train, self.X_test, self.y_test, self.params, self.verbose) # type: ignore
