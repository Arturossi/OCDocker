...
class PreXGBoostOptimizer:
    ...
    def objective(self, trial):
        ...
        -------
        float
            The RMSE of the model as a result of the suggested hyperparameters.
        """
        ...
        pruning_callback = XGBoostPruningCallback(trial, f"validation_0-auc")
        ...
        _, roc_auc = run_xgboost(self.X_train, self.y_train, self.X_test, self.y_test, trial_params, self.verbose) # type: ignore

        # Return the trained AUC score
        return roc_auc

    def optimize(self, direction: str = "maximize", n_trials: int = 1000,  n_jobs: int = 1, study_name: str = "XGBoost pre-optimization", load_if_exists: bool = True) -> tuple[optuna.study.Study, dict, float]:
        """
...
class EvolutionaryFeatureSelector:
    ...
    def objective_CMA(self, trial: optuna.Trial) -> float:
        ...
        _, roc_auc = run_xgboost(X_train_filtered, self.y_train, X_test_filtered, self.y_test, self.xgboost_params, self.verbose)
...
class EvolutionaryFeatureSelectorCustom:
    ...
    def fitness_function(self, features: list) -> float:
        ...
        # Train the model and get the AUC score
        _, roc_auc = run_xgboost(filtered_X_train, self.y_train, filtered_X_test, self.y_test, self.xgboost_params, self.verbose) # type: ignore
    ...
    def objective(self, trial: optuna.Trial) -> float:
        ...
        # Report the AUC score
        trial.report(best_score, trial.number)
