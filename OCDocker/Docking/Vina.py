    ...
    def run_rescore(self, outPath: str, logFile: str = "", skipDefaultScoring: bool = False, overwrite: bool = False) -> None:
        ...
        for scoring_function in config.vina.scoring_functions:
            ...
            if not (scoring_function == config.smina.scoring and skipDefaultScoring):
                ...
                _ = run_rescore(self.config, self.output_vina, outPath, scoring_function, logFile = logFile, splitLigand = splitLigand, overwrite = overwrite)
...
def run_rescore(confFile: str, ligands: Union[List[str], str], outPath: str, scoring_function: str, logFile: str = "", splitLigand: bool = True, overwrite: bool = False) -> None:
    ...
    for ligand in ligands:
        ...
        # Create the command list
        config = get_config()
        cmd = [config.vina.executable, "--scoring", scoring_function, "--autobox", "--score_only", "--config", confFile, "--ligand", ligand, "--dir", f"{outPath}", "--cpu", "1"]
