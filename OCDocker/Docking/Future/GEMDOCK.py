...
class GEMDOCK:
    ...
    def __init__(self, configPath: str, boxFile: str, receptor: ocr.Receptor, preparedReceptorPath: str, ligand: ocl.Ligand, preparedLigandPath: str, gemdockLog: str, outputGEMDOCK: str, name: str = "", overwriteConfig: bool = False, spacing: float = 2.9) -> None:
        ...
        if type(ligand) == ocl.Ligand:
            ...
            _ = ocff.safe_create_dir(os.path.join(os.path.dirname(ligand.path), "gemdockFiles"))
        else:
        ...
        # TODO: Change this
        self.prepareLigandCmd = ""

        self.active_site_pdb = f"{outputGEMDOCK}/receptor_active_site.pdb"

        # Write the active site to the outputGEMDOCK folder
        self.write_active_site()

        # Dock
        self.gemdockLog = str(gemdockLog)
        self.outputGEMDOCK = str(outputGEMDOCK)
        self.gemdockCmd = []
        ...
        # Aliases
        ############
        self.run_docking = self.run_gemdock
    ...
    def write_active_site(self):
        # Write the active site
        write_active_site(self.boxFile, self.inputReceptorPath, self.inputLigandPath, self.active_site_pdb, self.outputGEMDOCK, spacing = spacing)
...
## Public ##
def write_active_site(boxFile: str, receptor: str, ligand: str, active_site_pdb: str, outputPath: str, spacing: float = 2.9, radius: float = -1.0) -> None:
    ''' Write the active site of the receptor to a pdb and mol2 file using chimera. '''

    # Check if the radius is unset (negative)
    if radius < 0:
        # Get the binding site
        binding_site = get_binding_site(boxFile, spacing)

        # If the binding site is an int, it means that there was an error
        if isinstance(binding_site, int):
            # Print the error
            ocprint.print_error(f"Problems while getting the binding site: {binding_site}")

            # Return
            return None
    
        # Get the center (discard) and radius
        _, radius = binding_site

    # Change the extension of the active site .pdb to .mol2
    active_site_mol2 = os.path.splitext(active_site_pdb)[0] + ".mol2"

    # Create the script
    script = f"from chimera import runCommand as rc; rc('open {receptor} {ligand}'); rc('select #1 zr<{radius}'); rc('~select ::HOH'); rc('write format pdb selected #0 {active_site_pdb}'); rc('write format mol2 selected #0 {active_site_mol2}')"

    # Set the script path
    script_path = os.path.join(outputPath, 'write_active_site.py')

    # Set the command
    cmd = [chimera, "--nogui", "--script", script_path]

    with open(script_path, 'w') as script_file:
        script_file.write(script)

    # Run the command
    _ = ocrun.run(cmd, logFile = "", cwd = outputPath)
...
def box_to_gemdock(boxFile: str, confFile: str, receptor: str) -> int:
    '''Convert a box (DUDE like format) to GEMDOCK input.
