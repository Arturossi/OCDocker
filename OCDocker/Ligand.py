...
class Ligand:
    ...
    def create_box(self, centroid = None, savePath = "", boxLength = 2.9, overwrite = False):
        ...
        if not savePath:
            ...
            savePath = os.path.join(os.path.split(self.moleculePath)[0], 'boxes')
        ...
        if os.path.isfile(molecule):
            ...
            if extension == ".mol2":
                ...
                if extension == ".pdb":
                    ...
                    remover = SaltRemover()
                    # Load the molecule
                    m = rdkit.Chem.rdmolfiles.MolFromSmiles(smiles, sanitize = sanitize)
                    # Remove the salts
                    m = remover.StripMol(m)
                    # Add the hydrogens
                    m = Chem.AddHs(m)
                    # Embed the molecule
                    _ = AllChem.EmbedMolecule(m, AllChem.ETKDG())
...
def read_descriptors_from_json(path, returnDict = False):
    ...
    # Key error (when there is a missing key)
        ...
        octools.print_error(f"The following keys were not found in the json file '{k}'.")
