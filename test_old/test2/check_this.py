...
def predict_model(X):
    if isinstance(X, pd.DataFrame):
        X_tensor = torch.tensor(X.values, dtype=torch.float32).to(neural.device)
...
# Combine protein and label info for stratification
strat_labels = pd.Series(df_dudez['receptor']).astype(str) + "_" + pd.Series(df_dudez['type']).astype(str)

# Store mean SHAP values for each feature across runs
shap_means = defaultdict(list)
shap_all_values = []  # store all SHAP values for beeswarm plot

# Repeat SHAP analysis across multiple seeds for robustness
for seed in seed_list:
    sss = StratifiedShuffleSplit(n_splits=1, train_size=background_size, random_state=seed)
    ...
    # Compute SHAP values
    shap_values = explainer.shap_values(X_val.values, nsamples=background_size)
    shap_all_values.append(shap_values)

    # Store mean(|SHAP|) values for each feature
    mean_abs_shap = np.abs(shap_values).mean(axis=0)
    for i, fname in enumerate(X_val.columns):
        shap_means[fname].append(mean_abs_shap[i])

# Compute average SHAP importance across seeds
feature_names = list(shap_means.keys())
mean_importances = np.array([np.mean(shap_means[f]) for f in feature_names])

# Sort and plot averaged feature importance
...
plt.title("Top 20 Stable SHAP Features across Seeds")
plt.tight_layout()
plt.savefig("shap_stable_bar_plot.png", dpi=300, bbox_inches='tight')
plt.close()

# Beeswarm plot using SHAP's built-in function for last SHAP run
shap.summary_plot(shap_all_values[-1], X_val, feature_names=X_val.columns.tolist(), show=False)
plt.title("SHAP Beeswarm Plot (Last Run)")
plt.tight_layout()
plt.savefig("shap_stable_beeswarm_plot.png", dpi=300, bbox_inches='tight')
plt.close()
