...
def _install_workers_stubs(monkeypatch):
    ...
    ae_mod = types.ModuleType("OCDocker.OCScore.Optimization.legacy.models.dimensionality.AutoencoderOptimizer")
    ae_mod.AutoencoderOptimizer = _StubOptimizer  # type: ignore[attr-defined]

    ga_mod = types.ModuleType("OCDocker.OCScore.Optimization.legacy.models.dimensionality.GeneticAlgorithm")
    ga_mod.GeneticAlgorithm = _StubOptimizer  # type: ignore[attr-defined]

    dnn_mod = types.ModuleType("OCDocker.OCScore.Optimization.legacy.models.dnn.DNNOptimizer")
    dnn_mod.DNNOptimizer = _StubOptimizer  # type: ignore[attr-defined]

    trans_mod = types.ModuleType("OCDocker.OCScore.Optimization.legacy.models.transformer.TransOptimizer")
    trans_mod.TransOptimizer = _StubOptimizer  # type: ignore[attr-defined]

    xgb_mod = types.ModuleType("OCDocker.OCScore.Optimization.legacy.models.xgboost.XGBoostOptimizer")
    ...
    monkeypatch.setitem(sys.modules, "OCDocker.OCScore.Optimization.legacy.models.dimensionality.AutoencoderOptimizer", ae_mod)
    monkeypatch.setitem(sys.modules, "OCDocker.OCScore.Optimization.legacy.models.dimensionality.GeneticAlgorithm", ga_mod)
    monkeypatch.setitem(sys.modules, "OCDocker.OCScore.Optimization.legacy.models.dnn.DNNOptimizer", dnn_mod)
    monkeypatch.setitem(sys.modules, "OCDocker.OCScore.Optimization.legacy.models.transformer.TransOptimizer", trans_mod)
    monkeypatch.setitem(sys.modules, "OCDocker.OCScore.Optimization.legacy.models.xgboost.XGBoostOptimizer", xgb_mod)


def _import_workers(monkeypatch):
    _install_workers_stubs(monkeypatch)
    path = Path(__file__).resolve().parents[3] / "OCDocker" / "OCScore" / "Utils" / "legacy" / "Workers.py"
...
def test_ae_worker_covers_non_verbose_branches(monkeypatch, ocworkers):
    ...
    monkeypatch.setattr(ocworkers.LOGGER, "info", lambda *_a, **_k: print_calls.__setitem__("count", print_calls["count"] + 1))
...
def test_ga_worker_non_none_best_params_and_non_verbose(monkeypatch, ocworkers):
    ...
    monkeypatch.setattr(ocworkers.LOGGER, "info", lambda *_a, **_k: calls.__setitem__("prints", calls["prints"] + 1))
    monkeypatch.setattr(ocworkers.time, "sleep", lambda *_a, **_k: None)
...
def test_nn_ablation_worker_list_and_array_branches_with_verbose(monkeypatch, ocworkers):
    ...
    monkeypatch.setattr(ocworkers.time, "sleep", lambda *_a, **_k: None)
    monkeypatch.setattr(ocworkers.LOGGER, "info", lambda *_a, **_k: calls.__setitem__("printv", calls["printv"] + 1))
...
def test_ae_worker_retries_transient_optuna_storage_errors(monkeypatch, ocworkers):
    ...
    monkeypatch.setattr(ocworkers.LOGGER, "warning", lambda *_a, **_k: None, raising=False)
