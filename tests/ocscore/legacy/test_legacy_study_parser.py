...
def test_analyze_studies_old_tracks_flags_and_ablation_columns(monkeypatch):
    ...
    monkeypatch.setattr(ocstudy.optuna, "load_study", _load_study)
    monkeypatch.setattr(ocstudy.LOGGER, "info", lambda msg, *args: printed.append(msg % args if args else msg))
...
def test_analyze_studies_old_verbose_false_path_skips_print(monkeypatch):
    ...
    calls = {"printv": 0}
    monkeypatch.setattr(ocstudy.LOGGER, "info", lambda _msg: calls.__setitem__("printv", calls["printv"] + 1))
