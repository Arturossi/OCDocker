...
import OCDocker.OCScore.Optimization.legacy.models.dnn.DNNOptimizer as ocdnn
...
def test_dnnoptimizer_ablate_success_and_error_path(monkeypatch):
    ...
    monkeypatch.setattr(ocdnn.LOGGER, "exception", lambda msg, *args: msgs.append(msg % args if args else msg))
...
def test_dnnoptimizer_optimize_and_gpu_seed_branch(monkeypatch):
    ...
    logs = []
    monkeypatch.setattr(ocdnn.optuna, "create_study", _create_study)
    monkeypatch.setattr(ocdnn.LOGGER, "info", lambda msg, *args: logs.append(msg % args if args else msg))
