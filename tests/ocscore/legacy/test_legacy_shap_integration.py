...
###############################################################################
'''
Coverage tests for OCScore.Analysis.legacy.SHAP modules.
'''
...
def _load_shap_modules(monkeypatch):
    ...
    class _FakePermutationExplanation:
        def __init__(self, values):
            self.values = values

    class _FakePermutationExplainer:
        ...
        def __call__(self, x):
            _ = self.fn(np.asarray(x))
            return _FakePermutationExplanation(np.asarray(x))

    def _summary_plot(*_a, **_k):
        fake_shap_calls["summary"] += 1

    fake_shap.DeepExplainer = _FakeDeepExplainer
    fake_shap.GradientExplainer = _FakeGradientExplainer
    fake_shap.KernelExplainer = _FakeKernelExplainer
    fake_shap.PermutationExplainer = _FakePermutationExplainer
    fake_shap.summary_plot = _summary_plot
    monkeypatch.setitem(sys.modules, "shap", fake_shap)

    shap_pkg = importlib.import_module("OCDocker.OCScore.Analysis.legacy.SHAP")
    cli_mod = importlib.import_module("OCDocker.OCScore.Analysis.legacy.SHAP.Cli")
    data_mod = importlib.import_module("OCDocker.OCScore.Analysis.legacy.SHAP.Data")
    explain_mod = importlib.import_module("OCDocker.OCScore.Analysis.legacy.SHAP.Explain")
    model_mod = importlib.import_module("OCDocker.OCScore.Analysis.legacy.SHAP.Model")
    plots_mod = importlib.import_module("OCDocker.OCScore.Analysis.legacy.SHAP.Plots")
    runner_mod = importlib.import_module("OCDocker.OCScore.Analysis.legacy.SHAP.Runner")
    studies_mod = importlib.import_module("OCDocker.OCScore.Analysis.legacy.SHAP.Studies")
...
def test_shap_compute_values_deep_gradient_kernel_permutation_and_invalid(monkeypatch):
    mods = _load_shap_modules(monkeypatch)
    explain_mod = mods["explain"]
    ...
    out_gradient = explain_mod.compute_shap_values(
        neural=neural,
        X_background=X_background,
        X_eval=X_eval,
        explainer="gradient",
    ...
    out_permutation = explain_mod.compute_shap_values(
        neural=neural,
        X_background=X_background,
        X_eval=X_eval,
    ...
    assert out_gradient.shape == (2, 2)
    assert out_kernel.shape == (2, 2)
    assert out_permutation.shape == (2, 2)
