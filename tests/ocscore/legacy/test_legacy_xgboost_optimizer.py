...
def _load_xgb_optimizer_module(monkeypatch):
    ...
    xgb_run_mod = types.ModuleType("OCDocker.OCScore.Optimization.legacy.models.xgboost.OCxgboost")
    xgb_run_mod.run_xgboost = lambda *_a, **_k: (_FakeModel(), 0.5)  # type: ignore[attr-defined]
    monkeypatch.setitem(sys.modules, "OCDocker.OCScore.Optimization.legacy.models.xgboost.OCxgboost", xgb_run_mod)

    path = pkg_root / "OCScore" / "Optimization" / "legacy" / "models" / "xgboost" / "XGBoostOptimizer.py"
...
def test_optimize_creates_study_and_logs_when_verbose(monkeypatch, ocxgboptimizer):
    ...
    monkeypatch.setattr(ocxgboptimizer.LOGGER, "info", lambda msg, *args: captured["prints"].append(msg % args if args else msg))
