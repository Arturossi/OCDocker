...
def ocmods(monkeypatch):
    ...
    _ensure_package("OCDocker.OCScore.Utils.legacy", pkg_root / "OCScore" / "Utils" / "legacy")
    _ensure_package("OCDocker.OCScore.Optimization.legacy.models.xgboost", pkg_root / "OCScore" / "Optimization" / "legacy" / "models" / "xgboost")
    _ensure_package("OCDocker.OCScore.Optimization.legacy.models.transformer", pkg_root / "OCScore" / "Optimization" / "legacy" / "models" / "transformer")
    ...
    data_mod = types.ModuleType("OCDocker.OCScore.Utils.legacy.Data")
    data_mod.load_data = lambda **_k: {}  # type: ignore[attr-defined]
    monkeypatch.setitem(sys.modules, "OCDocker.OCScore.Utils.legacy.Data", data_mod)

    workers_mod = types.ModuleType("OCDocker.OCScore.Utils.legacy.Workers")
    ...
    monkeypatch.setitem(sys.modules, "OCDocker.OCScore.Utils.legacy.Workers", workers_mod)
    ...
    optuna_mod.create_study = lambda **_k: None  # type: ignore[attr-defined]
    optuna_mod.load_study = lambda **_k: None  # type: ignore[attr-defined]
    ...
    modules = {
        ...
        "pkg_opt_legacy_future": _load_module("OCDocker.OCScore.Optimization.legacy.future", pkg_root / "OCScore" / "Optimization" / "legacy" / "future" / "__init__.py"),
        "pkg_trans": _load_module("OCDocker.OCScore.Optimization.legacy.models.transformer", pkg_root / "OCScore" / "Optimization" / "legacy" / "models" / "transformer" / "__init__.py"),
        "pkg_xgb": _load_module("OCDocker.OCScore.Optimization.legacy.models.xgboost", pkg_root / "OCScore" / "Optimization" / "legacy" / "models" / "xgboost" / "__init__.py"),
        "dnn_opt": _load_module("OCDocker.OCScore.Optimization.legacy.DNN", pkg_root / "OCScore" / "Optimization" / "legacy" / "DNN.py"),
        "trans_opt": _load_module("OCDocker.OCScore.Optimization.legacy.Transformer", pkg_root / "OCScore" / "Optimization" / "legacy" / "Transformer.py"),
        "xgb_opt": _load_module("OCDocker.OCScore.Optimization.legacy.XGBoost", pkg_root / "OCScore" / "Optimization" / "legacy" / "XGBoost.py"),
        "xgb_run": _load_module("OCDocker.OCScore.Optimization.legacy.models.xgboost.OCxgboost", pkg_root / "OCScore" / "Optimization" / "legacy" / "models" / "xgboost" / "OCxgboost.py"),
    }
    return modules
...
def test_optimize_transformer_invalid_backend_raises(monkeypatch, ocmods):
    ...
    monkeypatch.setattr(octransopt.LOGGER, "warning", lambda msg: warnings.append(msg))
    monkeypatch.setattr(octransopt.LOGGER, "info", lambda _msg: None)
...
def test_optimize_xgb_invalid_backend_raises(monkeypatch, ocmods):
    ...
    monkeypatch.setattr(ocxgbopt.LOGGER, "warning", lambda msg: warnings.append(msg))
    monkeypatch.setattr(ocxgbopt.LOGGER, "info", lambda _msg: None)
...
def test_optimize_xgb_joblib_full_pipeline_uses_loaded_data(monkeypatch, ocmods):
    ...
    monkeypatch.setattr(ocxgbopt.LOGGER, "info", lambda *_a, **_k: None)
    monkeypatch.setattr(ocxgbopt.LOGGER, "warning", lambda *_a, **_k: None)
...
def test_optimize_nn_joblib_without_autoencoder_runs_nn_worker(monkeypatch, ocmods):
    ...
    monkeypatch.setattr(ocdnnopt.LOGGER, "info", lambda *_a, **_k: None)
    monkeypatch.setattr(ocdnnopt.LOGGER, "warning", lambda *_a, **_k: None)
