...
###############################################################################
'''
Coverage tests for OCScore.Optimization.legacy.Transformer runtime branches.
'''
...
def octrans(monkeypatch):
    ...
    return _load_module(
        ...
        pkg_root / "OCScore" / "Optimization" / "legacy" / "Transformer.py",
    )
...
def test_optimize_transformer_joblib_and_load_data(monkeypatch, octrans):
    ...
    monkeypatch.setattr(octrans.LOGGER, "warning", lambda msg: calls["warnings"].append(msg))
    monkeypatch.setattr(octrans.LOGGER, "info", lambda msg: calls["verbose"].append(msg))
...
def test_optimize_transformer_multiprocessing_and_skip(monkeypatch, octrans):
    ...
    monkeypatch.setattr(octrans.LOGGER, "warning", lambda *_a, **_k: None)
    monkeypatch.setattr(octrans.LOGGER, "info", lambda *_a, **_k: None)
