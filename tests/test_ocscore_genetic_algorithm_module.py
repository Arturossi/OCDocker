...
def ocga(monkeypatch):
    ...
    _ensure_package("OCDocker.OCScore.Dimensionality", pkg_root / "OCScore" / "Dimensionality")
    ...
    xgb_calls = {"run": 0}
    xgb_mod = types.ModuleType("OCDocker.OCScore.XGBoost.OCxgboost")
    ...
    monkeypatch.setitem(sys.modules, "OCDocker.OCScore.XGBoost.OCxgboost", xgb_mod)

    module = _load_module(
        "OCDocker.OCScore.Dimensionality.GeneticAlgorithm",
        pkg_root / "OCScore" / "Dimensionality" / "GeneticAlgorithm.py",
    )
