...
###############################################################################
'''
Coverage tests for OCScore.Optimization.future.DNN helpers.
'''
...
def ocfuture(monkeypatch):
    ...
    module = _load_module(
        "OCDocker.OCScore.Optimization.future.DNN",
        pkg_root / "OCScore" / "Optimization" / "future" / "DNN.py",
    )
...
def test_optimize_future_fallback_and_invalid_backend(ocfuture, monkeypatch):
    ...
    fallback_mod = types.ModuleType("OCDocker.OCScore.Optimization.DNN")
    fallback_mod.optimize_NN = lambda **kwargs: ("fallback", kwargs["storage_id"])  # type: ignore[attr-defined]
    monkeypatch.setitem(sys.modules, "OCDocker.OCScore.Optimization.DNN", fallback_mod)
