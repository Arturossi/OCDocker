    ...
    for metric in metrics:
        ...
        for plot_type in ['boxplot', 'violin']:
            ...
            for plot, ax in zip(['RMSE', 'AUC'], [ax1, ax2]):
                ...
                # Add mean lines
                if plot == 'RMSE':
                    ax.axhline(mean_rmse, color='red', linestyle='--', label='Mean RMSE consensus')
                else:
                    ax.axhline(mean_auc, color='blue', linestyle='--', label='Mean AUC consensus')
                ...
                # Highlight shaded areas for method types
                unique_methods = data['Methodology'].unique().tolist()

                nn_methods = [m for m in unique_methods if m.startswith('NN')]
                xgb_methods = [m for m in unique_methods if m.startswith('XGB')]
                transformer_methods = [m for m in unique_methods if m.startswith('Transformer')]

                for method_group, color in zip([nn_methods, xgb_methods, transformer_methods], ['lightblue', 'lightgreen', 'lightcoral']):
                    for method in method_group:
                        method_pos = unique_methods.index(method)
                        ax.axvspan(method_pos - 0.5, method_pos + 0.5, color=color, alpha=0.3)

                # Add combined legend (metrics + shaded)
                custom_patches = [
                    Patch(facecolor='lightblue', edgecolor='black', alpha=0.3, label='Neural Network'),
                    Patch(facecolor='lightgreen', edgecolor='black', alpha=0.3, label='XGBoost'),
                    Patch(facecolor='lightcoral', edgecolor='black', alpha=0.3, label='Transformer')
                ]
                ax.legend(handles=custom_patches + ax.get_legend_handles_labels()[0], loc='best')

            # Better title
            fig.suptitle(f'{aux_metric} Distribution (500 results per methodology, top {n_trials} trials)', fontsize=16)
